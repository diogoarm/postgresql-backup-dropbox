...
databases=$(psql $POSTGRESQL_HOST_OPTS -c "SELECT datname FROM pg_database WHERE datistemplate = false;")
...
do
  ...
  then
    ...
    pg_dump $POSTGRESQL_HOST_OPTS $POSTGRESQLDUMP_OPTIONS $database \
      | gzip \
      | curl -X PUT \
